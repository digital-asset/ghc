...
importdecl :: { LImportDecl GhcPs }
        : 'import' maybe_src maybe_safe optqualified maybe_pkg modid optqualified maybeas maybeimpspec
                {% do
                  ...
                  ;  if preposQualMod then warnPrepositiveQualifiedModule (getLoc $4)
                     else return ()
...
ensure_decl :: { LHsExpr GhcPs }
  : 'ensure' exp                                 {% runExpCmdP $2 >>= \ $2 ->  return $ sLL $1 $> $ unLoc $2 }
...
parties :: { Located [LHsExpr GhcPs] }
  ...
  | exp                                          {% runExpCmdP $1 >>=  \ $1 -> return $ sL1 $1 [applyToParties $1] }
...
exp   :: { ExpCmdP }
        : infixexp OF_TYPE sigtype {% runExpCmdP $1 >>= \ $1 ->
                                   fmap ecFromExp $
