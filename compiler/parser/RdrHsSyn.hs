...
checkValDef strictness lhs msig g@(dL->L l (_,grhss))
        ...
        ; case mb_fun of
            ...
            Nothing ->
              case msig of
                ...
                Just sig -> do
                  lhs' <- runPV $ mkHsTySigPV (combineLocs lhs sig) lhs sig >>= checkLPat
                  checkPatBind lhs' g }
...
checkFunBind :: SrcStrictness
             ...
             -> P ([AddAnn],HsBind GhcPs)
checkFunBind strictness ann lhs_loc fun is_infix pats msig (dL->L rhs_span grhss)
  = do
        pState <- getPState
...
instDecl :: ClsInstDecl GhcPs -> P ([LHsDecl GhcPs])
instDecl cid = do
...
mkTemplateTypeDecl
  ...
  let eqName = L nloc $ qualifyDesugar $ mkClsOcc "Eq"
      eqTyCl = mkLHsSigType $ L nloc $ HsTyVar noExt NotPromoted eqName
  let showName = L nloc $ qualifyDesugar $ mkClsOcc "Show"
