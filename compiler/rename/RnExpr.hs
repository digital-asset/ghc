...
rnExpr (RecordUpd { rupd_expr = expr, rupd_flds = Right flds})
  = do { ; unlessXOptM LangExt.RebindableSyntax $
         ...
         ; (getField, fv_getField) <- lookupSyntaxName0 getFieldName
         ; (setField, fv_setField) <- lookupSyntaxName0 setFieldName
         ; (e, fv_e) <- rnLExpr expr
...
-- TODO (drsk) what to do with missing HsExpansion and rebindable syntax?
